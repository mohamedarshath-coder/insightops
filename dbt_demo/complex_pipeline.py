...
df_customers = df_customers.repartition(100)
...
df_result = df_joined.withColumn("sum_order_amount", F.sum("order_amount").over(window))
df_result = df_result.groupBy("sales_region").agg(F.sum("sum_order_amount").alias("total_order_amount"))
